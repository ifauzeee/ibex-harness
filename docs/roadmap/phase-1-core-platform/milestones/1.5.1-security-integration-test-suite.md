...
|---|---|---|---|
| SEC-1.1 | No Authorization header | — | 401 `UNAUTHENTICATED` |
...
| SEC-3.4 | Response to SEC-3.2 does NOT differ in timing from SEC-3.3 | Timing delta < 5ms |
...
| Test ID | Scenario | Expected |
|---|---|---|
| SEC-5.1 | Token with no permissions (`permissions=0`) | 403 `PERMISSION_DENIED` |
...
- [ ] All 31 test cases in the matrix above pass with `-tags=integration`
- [ ] SEC-3.4: timing delta between cross-org denials < 5ms (prevents timing side-channel)
