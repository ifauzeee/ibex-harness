...
Milestones 1.1.1 and 1.1.2 delivered Postgres `ibex_core.tokens` with RLS and the `ibex.auth.v1` `ValidateToken` gRPC contract ([ADR-0006](ADR-0006-auth-proto-contract.md)). Milestone 1.1.3 implements the auth service server consumed by the proxy ([1.2.1](../roadmap/phase-1-core-platform/milestones/1.2.1-proxy-auth-client.md)).

[SECURITY.md](../SECURITY.md) requires Argon2id for stored tokens, fail-closed behavior, and no raw tokens in logs. [TESTING_STRATEGY.md](../TESTING_STRATEGY.md) §6.2 defines auth unit and integration expectations.
...
3. Open DB transaction with `SET LOCAL app.is_service_account = 'true'` ([ADR-0005](ADR-0005-postgres-migration-strategy.md))
...
Generated stubs: `make proto-gen` → `packages/proto/gen/go/ibex/auth/v1` (gitignored per [ADR-0004](ADR-0004-protobuf-and-codegen-policy.md)).
...
- [Milestone 1.1.3](../roadmap/phase-1-core-platform/milestones/1.1.3-auth-token-validation.md)
- [ARCHITECTURE.md](../ARCHITECTURE.md) — auth validation pipeline
