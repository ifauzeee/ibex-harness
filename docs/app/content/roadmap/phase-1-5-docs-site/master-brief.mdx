---
title: "IBEX Harness"
description: "> Single source of truth for designing, building and shipping > `docs.ibexharness.com`. This document supersedes > `BRAND_LANDING_DOCS_DOMAIN_SETUP.md` for everything *docs-site* related. > The landing page is already built in a separate repository > (this Lovable project) and is referenced here only at the boundar"
summary: "> Single source of truth for designing, building and shipping > `docs.ibexharness.com`. This document supersedes > `BRAND_LANDING_DOCS_DOMAIN_SETUP.md` for everything *docs-site* related. > The landing page is already built in a separate repository > (this Lovable project) and is referenced here only at the boundar"
...
> **Original brief:** Vercel project `ibex-harness-docs`. **Superseded** by Cloudflare Workers + GitHub Actions deploy — see [ADR-0023](/docs/adr/0023-docs-site-architecture) and [`DEPLOY_CLOUDFLARE.md`](../../../DEPLOY_CLOUDFLARE.md).

- [@opennextjs/cloudflare](https://opennext.js.org/cloudflare) bundles Next.js 15 for Workers.
- Deploy only via `.github/workflows/docs-deploy.yml` (not Cloudflare Workers Builds Git).
- Preview validation: `docs-build` CI on PRs + optional `workflow_dispatch` deploy smoke test.
...
- [ ] `docs/app/content/roadmap/CURRENT_STATE` updated:
