---
title: "Phase 1 — Risks"
description: "- Bloom filter for revoked tokens (architecture mentions; Phase 2) - JWT session tokens for dashboard (Phase 3+ / separate track) - `make db-reset` destructive helper (only if safely documented)"
summary: "- Bloom filter for revoked tokens (architecture mentions; Phase 2) - JWT session tokens for dashboard (Phase 3+ / separate track) - `make db-reset` destructive helper (only if safely documented)"
