...
for entry in *; do
  if [[ -d "$entry" ]]; then
    ...
    case "$entry" in
      .cursorrules|.editorconfig|.gitattributes|.gitignore|.markdownlint-cli2.jsonc|.gitleaks.toml|.golangci.yml|.pre-commit-config.yaml|codecov.yml|Makefile|go.mod|go.sum|package.json|pnpm-lock.yaml|pnpm-workspace.yaml|turbo.json|.nvmrc|LICENSE|AGENTS.md|CODE_OF_CONDUCT.md|CONTRIBUTING.md|PROMPTS.md|README.md|node_modules) ;;
      *)
