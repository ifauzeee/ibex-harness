---
title: "ADR-0006: Auth protobuf contract (`ibex.auth.v1`)"
description: "Architecture decision record 0006."
adrId: "0006"
status: "Accepted"
date: "2026-06-03"
...
[ADR-0004](/docs/adr/0004-protobuf-and-codegen-policy) defines Buf lint/breaking and uncommitted `gen/`. [ARCHITECTURE.md](/docs/architecture) documents the auth validation pipeline: proxy calls auth on cache miss and expects `org_id`, optional `agent_id`, permission bitmap, and expiry.
...
- **Layout:** Same pattern as `ibex.context.v1` under `packages/proto/proto/ibex/<domain>/v1/` ([FILE_STRUCTURE.md](/docs/file_structure) §4.1)
...
- **RPCs:**
  ...
  - `CreateToken` — returns plaintext **once**; requires caller bearer with `TokenCreate` ([ADR-0009](/docs/adr/0009-permission-bitmap))
...
- `permissions` (int64): 64-bit permission bitmap per [ARCHITECTURE.md](/docs/architecture)
...
- [ADR-0004](/docs/adr/0004-protobuf-and-codegen-policy)
- [ADR-0005](/docs/adr/0005-postgres-migration-strategy)
- [Milestone 1.1.2](/roadmap/phase-1-core-platform/milestones/1.1.2-auth-proto-and-codegen)
