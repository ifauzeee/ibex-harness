...
<Callout type="info" title="Phase 2 progress">
  Milestone [2.1.1 Provider interface and registry](/roadmap/phase-2-single-provider/milestones/2.1.1-provider-interface-and-registry) ships `packages/provider` (interface + registry) per [ADR-0025](/docs/adr/0025-llm-provider-abstraction). The first OpenAI adapter lands in milestone **2.1.2**.
</Callout>
