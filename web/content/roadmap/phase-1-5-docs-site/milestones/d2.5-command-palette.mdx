---
title: "d2.5 Command Palette"
description: "Docs IA will grow to 30+ pages after D.3.1. Sidebar-only navigation does not scale for returning users who need heading-level jumps (e.g. \\\"rate limit\\\", \\\"RLS\\\"). Fumadocs Orama search must index `structuredData` from each page so Cmd+K returns heading matches, not just page titles."
summary: "Docs IA will grow to 30+ pages after D.3.1. Sidebar-only navigation does not scale for returning users who need heading-level jumps (e.g. \\\"rate limit\\\", \\\"RLS\\\"). Fumadocs Orama search must index `structuredData` from each page so Cmd+K returns heading matches, not just page titles."
status: "completed"
milestoneId: "d2.5"
goal: "[D.2](/roadmap/phase-1-5-docs-site/goals)"
estimatedEffort: "2 hours"
phase: "phase-1-5-docs-site"
fullTitle: "Milestone D.2.5 — Cmd+K command palette"
---
...
Spec detail: [MASTER_BRIEF.md](/roadmap/phase-1-5-docs-site/master-brief) Part D (D.2.5).
...
| --- | --- | --- |
| `web/src/lib/search.ts` | Add/Modify | Orama advanced search API |
| `web/src/components/search-dialog.tsx` | Modify | Matte Graphite dialog skin |
| `web/src/components/layout/nav-search.tsx` | Modify | Kbd hint in nav |
