...
**ADR required:** ADR-0025 — LLM provider abstraction design
...
## ADR-0025 — Provider abstraction design

Write `web/content/docs/adr/0025-llm-provider-abstraction.mdx` documenting:
...
- [x] `packages/provider.Provider` interface defined with `Complete`, `Name`, `SupportedModels`
- [x] `packages/provider.Registry` selects provider by model ID
- [x] `ErrNoProviderForModel` triggers `501 PROVIDER_NOT_CONFIGURED` response in proxy handler
- [x] ADR-0025 written and indexed
- [x] Interface has no concrete OpenAI imports (pure abstraction)

---

## Prerequisites

- Phase 1 + 1.5 exit (merged)
- `packages/apierror` `CodeProviderNotConfigured` exists

## Testing Requirements

- `go test ./packages/provider/...` (unit, offline)
- `go test ./services/proxy/internal/http/...` (handler still returns 501)
- `gofmt -l` on touched `.go` files (empty = pass)
- `make repo-guards`
...
| Path | Change |
| --- | --- |
| `packages/provider/*` | New |
| `services/proxy/internal/http/router.go` | Registry lookup |
| `services/proxy/cmd/proxy/main.go` | Empty registry init |
| `web/content/docs/adr/0025-*` | New ADR |