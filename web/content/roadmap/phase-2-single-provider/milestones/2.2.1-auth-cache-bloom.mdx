...
description: "The Phase 1 proxy makes a synchronous gRPC call to the auth service on every protected request. This call has a 50ms budget (ARCHITECTURE.md) and in practice takes 2–10ms under normal load. That is already 10–50% of the 20ms proxy overhead budget, leaving little room for directive resolution, session writes, and trace"
summary: "The Phase 1 proxy makes a synchronous gRPC call to the auth service on every protected request. This call has a 50ms budget (ARCHITECTURE.md) and in practice takes 2–10ms under normal load. That is already 10–50% of the 20ms proxy overhead budget, leaving little room for directive resolution, session writes, and trace"
status: "planned"
milestoneId: "2.2.1"
...
**ADR required:** ADR-0028 — Auth cache design and revocation SLA
...
Write `web/content/docs/adr/0028-auth-cache-design.mdx` covering:
