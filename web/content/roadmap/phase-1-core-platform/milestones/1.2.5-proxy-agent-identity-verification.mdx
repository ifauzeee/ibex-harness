---
title: "1.2.5 Proxy Agent"
description: "Milestone M1.2.3 added extraction of `X-IBEX-Agent-ID` from the HTTP header and attaches the raw UUID to the request context. It performs no validation beyond \\\"is this a parseable UUID.\\\" The result is a concrete security gap: an authenticated token from Org A combined with the UUID of Org B's agent produces a request c"
summary: "Milestone M1.2.3 added extraction of `X-IBEX-Agent-ID` from the HTTP header and attaches the raw UUID to the request context. It performs no validation beyond \\\"is this a parseable UUID.\\\" The result is a concrete security gap: an authenticated token from Org A combined with the UUID of Org B's agent produces a request c"
status: "completed"
milestoneId: "1.2.5"
...
fullTitle: "Milestone 1.2.5 — Agent Identity Verification in Proxy Middleware"
---
...
- [1.1.7](/roadmap/phase-1-core-platform/milestones/1.1.7-users-and-agents-schema) merged — `agents` table exists, `ValidateAgent` gRPC implemented in auth service
- [1.2.1](/roadmap/phase-1-core-platform/milestones/1.2.1-proxy-auth-client) merged — auth gRPC client pool exists in proxy
- [1.2.3](/roadmap/phase-1-core-platform/milestones/1.2.3-proxy-input-validation-and-stable-error-envelope) merged — `X-IBEX-Agent-ID` extraction exists
...
| `web/engineering/SECURITY.md` | Document agent identity verification in auth flow |
