---
title: "1.3.3 Shared Logger"
description: "ARCHITECTURE.md defines a mandatory JSON log schema. Two services rolling their own logging independently will diverge: different field names, inconsistent severity levels, no guarantee that `request_id`, `org_id`, or `trace_id` appear in every log line. A bug that leaks `org_id` into a log message's string body rather"
summary: "ARCHITECTURE.md defines a mandatory JSON log schema. Two services rolling their own logging independently will diverge: different field names, inconsistent severity levels, no guarantee that `request_id`, `org_id`, or `trace_id` appear in every log line. A bug that leaks `org_id` into a log message's string body rather"
status: "completed"
milestoneId: "1.3.3"
...
fullTitle: "Milestone 1.3.3 — Shared Structured Logger Package (`packages/logger`)"
---
...
- [1.2.6](/roadmap/phase-1-core-platform/milestones/1.2.6-request-id-correlation-middleware) merged — `reqid.FromContext` available
